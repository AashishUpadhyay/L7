...
describe('ActorPage', () => {
  ...
  it('opens movies modal when movie count is clicked', async () => {
    vi.mocked(personsApi.getPersonMovies).mockResolvedValue([
      { id: 1, movie_id: 1, movie_title: 'Inception', role: 'Actor', image_path: null, rating: 8.8, release_date: '2010-07-16', genres: [1, 5] },
      { id: 2, movie_id: 2, movie_title: 'Titanic', role: 'Director', image_path: null, rating: 7.9, release_date: '1997-12-19', genres: [3, 8] },
    ])
    renderPage()
    await waitFor(() => {
      expect(screen.getByRole('button', { name: '2 movies' })).toBeInTheDocument()
