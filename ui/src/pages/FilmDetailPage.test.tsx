...
function renderWithRouter(id: string) {
  return render(
    <MemoryRouter initialEntries={[`/admin/movies/${id}`]}>
      <Routes>
        <Route path="/admin/movies/:id" element={<FilmDetailPage />} />
      </Routes>
    </MemoryRouter>
  )
}
...
describe('FilmDetailPage', () => {
  ...
  it('renders breadcrumbs with Home, Film, and title', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Manage Movies' })).toHaveAttribute('href', '/admin/movies')
  })
