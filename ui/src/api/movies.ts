...
import type { ReviewListResponse, ReviewCreate, Review } from '@/types/review'
...
export function getMovieReviews(movieId: number, skip = 0, limit = 20) {
  return request<ReviewListResponse>(`/movies/${movieId}/reviews`, {
    params: { skip, limit },
  })
}

export function createMovieReview(movieId: number, payload: ReviewCreate) {
...
export function deleteMovieReview(movieId: number, reviewId: number) {
